...
app.get('/play/:quarkUrl/:index', async (req, res) => {
  try {
    ...
    if (directLinks && directLinks.length > 0 && directLinks[index]) {
      ...
      const proxyUrl = `${baseUrl}/proxy/play?url=${encodeURIComponent(directLink)}`;
      log(`重定向到代理: ${proxyUrl.substring(0, 80)}...`);
...
app.get('/api.php/provide/vod/play', async (req, res) => {
  try {
    ...
    if (targetEpisodes.length > 0) {
      ...
      const baseUrl = getBaseUrl(req);
      const proxyUrl = `${baseUrl}/proxy/play?url=${encodeURIComponent(targetEpisode.url)}`;
      return res.redirect(proxyUrl);
    } else {
      log(`未找到对应集数: ${episode}`);
...
app.get('/proxy/play', async (req, res) => {
  try {
    ...
    const ignoreHeaders = ['host', 'connection', 'accept-encoding', 'authorization', 'referer'];
    ...
    while (redirectCount < maxRedirects) {
      ...
      if (response.status >= 300 && response.status < 400 && response.headers.get('location')) {
        ...
        log(`重定向 ${redirectCount + 1}: ${currentUrl.substring(0, 60)}... -> ${location.substring(0, 60)}...`);
        currentUrl = location;
        redirectCount++;
        continue;
      }
    ...
    if (redirectCount >= maxRedirects) {
      log(`重定向次数过多: ${redirectCount}`);
      return res.status(508).send('重定向次数过多');
    }
