...
function getQuarkPlayTempSavePath() {
  return process.env.QUARK_PLAY_TEMP_SAVE_PATH || config.quarkPlayTempSavePath || '/quark-temp';
}

function getOpenlistTempPath() {
  return process.env.OPENLIST_TEMP_PATH || config.openlistTempPath || '/quark-temp';
}
  ...
  <div class="container">
    ...
    <div class="card">
      ...
      <div class="form-group">
        ...
        <input type="text" id="openlistUrl" placeholder="http://localhost:3000">
    ...
    <div class="card">
      ...
      <div class="form-group">
        <label>夸克转存目录</label>
        <input type="text" id="quarkPlayTempSavePath" placeholder="/quark-temp">
...
app.post('/api/quark/instant-play', async (req, res) => {
  try {
    ...
    const cookie = getQuarkCookie();
    if (!cookie) {
      return res.status(400).json({ error: '夸克网盘未配置' });
    }
...
app.get('/play/:quarkUrl/:index', async (req, res) => {
  try {
    ...
    if (videoFiles.length > 0 && videoFiles[index]) {
      ...
      const openlistPlayUrl = `${openlistUrl}${openlistFolderPath}/${encodeURIComponent(targetFile.name)}`;
      ...
      if (ids.startsWith('all:')) {
        ...
        if (isMovie) {
          ...
          for (let i = 0; i < allUrls.length; i++) {
            ...
            try {
              const directLinks = await getDirectLink(cleanUrl, true, true, req);
