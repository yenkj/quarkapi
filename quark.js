...
class QuarkDirectLink {
  ...
  async getTaskStatus(taskId, retryIndex = 0) {
    const t = Date.now();
    const url = `${QUARK_PC_API}/task?pr=ucpro&fr=pc&uc_param_str=&task_id=${taskId}&retry_index=${retryIndex}&__dt=${t % 1000}&__t=${t}`;
    ...
    if (!data.data) {
      throw new Error(`任务状态查询失败: ${JSON.stringify(data)}`);
    }
  ...
  async waitForTask(taskId, maxRetries = 30, interval = 2000) {
    for (let i = 0; i < maxRetries; i++) {
      const taskInfo = await this.getTaskStatus(taskId, i);
  ...
  async getDirectLinks(shareUrl, passcode = '') {
    ...
    console.log('task_id:', saveResult.task_id);
